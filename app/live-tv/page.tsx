...
export default function LiveTVPage() {
  ...
  useEffect(() => {
    if (id) {
      ...
      const filteredChannels1 = channels.filter((ch) => ch.id == id);
      console.log("test",filteredChannels1)
      setActiveChannel(filteredChannels1[0]);
    }
  }, [id, channels]);
        ...
        <div className="bg-slate-950 pt-16 min-h-screen flex flex-col">
          ...
          <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
            <div className="flex-1 flex flex-col min-w-0">
              ...
              allowFullScreen
            /> */}
                <HLSPlayer src={activeChannel.link} />
              ...
              <div className="bg-slate-900 border-t border-slate-800 p-4 sm:p-6">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-4 min-w-0">
                    <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-white text-lg font-black flex-shrink-0 shadow-lg">
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  {["HD", "16:9", "Stereo", "Live Stream"].map((tag) => (
                    <span
                      key={tag}
                      className="text-slate-500 text-xs bg-slate-800 px-2.5 py-1 rounded-lg"
                    >
                      {tag}
                    </span>
                  ))}
                  ...
                  {channels.slice(0, 10).map((ch) => (
                    ...
                    >
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white text-sm font-bold">
                        <img
                          src={
                            "https://www.codeminer.in/tvchanel/admin/upload/" +
                            ch.logo
                          }
                        />
                      </div>
                ...
                {filteredChannels.map((ch) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-slate-800/60 transition-colors text-left group ${
                      activeChannel.id === ch.id
                        ? "bg-slate-800 border-l-2 border-red-500"
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <span
                        className={`text-sm font-medium truncate block ${activeChannel.id === ch.id ? "text-white" : "text-slate-300 group-hover:text-white"}`}
                      >
                        {ch.name}
                      </span>
                      <span className="text-slate-600 text-xs truncate block mt-0.5">
                        {ch.category}
                      </span>
                    </div>
